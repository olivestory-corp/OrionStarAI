...
/*
 * -------------------------------------------------------------------------
 *  Paste handling utilities for intelligent text processing
 * ---------------------------------------------------------------------- */

/**
 * 检测文本是否包含真正的换行符（区分真正的换行符和字面量字符串）
 */
export function hasRealLineBreaks(content: string): boolean {
  return /[\r\n]/.test(content);
}

/**
 * 获取真实的行数（处理不同操作系统的换行符）
 */
export function getRealLineCount(content: string): number {
