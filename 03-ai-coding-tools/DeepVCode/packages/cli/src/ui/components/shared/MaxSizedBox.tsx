...
export const MaxSizedBox: React.FC<MaxSizedBoxProps> = ({
  ...
  // 🔧 调试信息：记录MaxSizedBox的参数
  if (enableDebugLog) {
    console.log(`[MaxSizedBox Debug] maxWidth: ${maxWidth}, maxHeight: ${maxHeight}, targetMaxHeight: ${targetMaxHeight}`);
  }
  ...
  // 🔧 调试信息：记录可见内容
  if (enableDebugLog) {
    console.log(`[MaxSizedBox Debug] laidOutStyledText.length: ${laidOutStyledText.length}, visibleStyledText.length: ${visibleStyledText.length}, hiddenLinesCount: ${hiddenLinesCount}`);
    if (visibleStyledText.length === 0) {
      console.log(`[MaxSizedBox Debug] ❌ NO VISIBLE CONTENT!`);
    } else {
      console.log(`[MaxSizedBox Debug] First visible line: "${visibleStyledText[0]?.map(seg => seg.text).join('') || 'EMPTY'}"`);
    }
  }
  ...
  // 🔧 修复：移除任意宽度限制，使用传入的精确宽度以确保边框对齐
  const safeMaxWidth = maxWidth; // 使用精确宽度，不再限制最大宽度

  if (enableDebugLog) {
    console.log(`[MaxSizedBox Debug] Using maxWidth: ${safeMaxWidth} (no arbitrary limit)`);
  }

  return (
    <Box flexDirection="column" width={safeMaxWidth} flexShrink={0}>
