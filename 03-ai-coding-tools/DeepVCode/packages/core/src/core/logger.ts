...
export class Logger {
  ...
  async saveCheckpoint(conversation: Content[], tag: string): Promise<void> {
    if (!this.initialized) {
      console.error('[CHECKPOINT DEBUG] Logger not initialized, cannot save checkpoint');
      return;
    }
    ...
    try {
      ...
      console.error('[CHECKPOINT DEBUG] Failed to write checkpoint file:', error);
    }
  }

  async loadCheckpoint(tag: string): Promise<Content[]> {
    if (!this.initialized) {
      console.error('[CHECKPOINT DEBUG] Logger not initialized, cannot load checkpoint');
      return [];
    }
    ...
    try {
      ...
      if (!Array.isArray(parsedContent)) {
        console.warn(`[CHECKPOINT DEBUG] Invalid checkpoint format at ${path}`);
        return [];
      }
      ...
      console.error(`[CHECKPOINT DEBUG] Failed to load checkpoint:`, error);
      return [];
    }
  }
