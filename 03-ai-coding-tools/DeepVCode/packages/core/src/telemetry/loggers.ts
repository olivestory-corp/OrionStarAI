...
export function logApiError(config: Config, event: ApiErrorEvent): void {
  ...
  // 将 agent_context 展开为简单属性
  if (event.agent_context) {
    attributes.agent_type = event.agent_context.type;
    if (event.agent_context.agentId) {
      attributes.agent_id = event.agent_context.agentId;
    }
    if (event.agent_context.taskDescription) {
      attributes.task_description = event.agent_context.taskDescription;
    }
  }
