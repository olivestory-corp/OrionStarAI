...
`;
    ...
    it('should handle Windows absolute paths outside project without crashing', () => {
      // Test the specific case that was causing the RangeError
      const windowsAbsolutePath = 'C:/Users/user/Pictures/image.jpg';
      expect(() => parser.isIgnored(windowsAbsolutePath)).not.toThrow();
      expect(parser.isIgnored(windowsAbsolutePath)).toBe(false);
    });

    it('should handle Unix absolute paths outside project without crashing', () => {
      const unixAbsolutePath = '/home/user/documents/file.txt';
      expect(() => parser.isIgnored(unixAbsolutePath)).not.toThrow();
      expect(parser.isIgnored(unixAbsolutePath)).toBe(false);
    });

    it('should handle complex project-external paths that would produce ".." relative paths', () => {
      // This would produce a relativePath like "../../../Users/user/file.txt"
      const complexPath = path.resolve(projectRoot, '../../../Users/user/file.txt');
      expect(() => parser.isIgnored(complexPath)).not.toThrow();
      expect(parser.isIgnored(complexPath)).toBe(false);
    });
  });
