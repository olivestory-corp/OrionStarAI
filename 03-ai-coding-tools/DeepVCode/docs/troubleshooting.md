...
## SSH/WSL Environment Issues

- **Problem: Arrow keys display as escape sequences (^[[A, ^[[B, etc.) in interactive menus**
  ...
  - **Solution:**
    ...
    3. **Terminal type:** Ensure proper terminal type is set:
       ...
       ```
  - **See:** [SSH/WSL Compatibility Guide](./ssh-wsl-compatibility.md) for detailed troubleshooting.

- **Problem: Slow response or input lag in SSH connections**
  - **Cause:** Network latency or terminal configuration issues.
  - **Solution:**
    1. Test network latency to the server
    2. Try different terminal types (`screen-256color`, `linux`)
    3. Enable SSH compression: `ssh -C user@server`

- **Problem: WSL-specific keyboard issues**
