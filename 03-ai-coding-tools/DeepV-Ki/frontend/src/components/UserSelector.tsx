...
*.pptx`;
  ...
  const getProviderDisplayName = (provider: Provider) => {
    // 1. Try translation
    const translationKey = `provider${provider.id.charAt(0).toUpperCase() + provider.id.slice(1)}`;

    const formMessages = t.form as Record<string, string> | undefined;
    if (formMessages?.[translationKey]) {
      return formMessages[translationKey];
    }

    // 2. Known providers mapping (Hardcoded fallback)
    ...
    if (displayNames[provider.id.toLowerCase()]) {
      return displayNames[provider.id.toLowerCase()];
    }

    // 3. Fallback to ID capitalization (Better than the wrong 'name' from backend)
    return provider.id.charAt(0).toUpperCase() + provider.id.slice(1);
  };
