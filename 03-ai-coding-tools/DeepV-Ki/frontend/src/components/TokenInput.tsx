      ...
      {showTokenSection && (
        <div className="mt-2 p-4 bg-[var(--background)]/50 rounded-md border border-[var(--border-color)]">
          ...
          <div>
            ...
            />
            <div className="space-y-2 mt-2 text-xs text-[var(--muted)]">
              ...
              {selectedPlatform === 'gitlab' && (
                <div className="flex items-start">
                  <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 mr-1 mt-0.5 text-orange-500 flex-shrink-0"
                    fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2}
                      d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                  </svg>
                  <div>
                    <div>{t.form?.gitlabTokenHelp || 'GitLab token requires \'read_repository\' permission'}</div>
                    <div className="mt-1">{t.form?.gitlabSelfHostedNote || 'Supports self-hosted GitLab instances'}</div>
                  </div>
                </div>
              )}
              ...
              {selectedPlatform === 'github' && (
                <div className="flex items-center">
                  <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 mr-1 text-blue-500"
                    fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2}
                      d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                  </svg>
                  {t.form?.githubTokenHelp || 'GitHub token requires \'repo\' permission (for private repos)'}
                </div>
              )}
              
              {selectedPlatform === 'bitbucket' && (
                <div className="flex items-center">
                  <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 mr-1 text-blue-600"
                    fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2}
                      d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                  </svg>
                  {t.form?.bitbucketTokenHelp || 'Bitbucket token requires \'Repositories: Read\' permission'}
                </div>
              )}
            </div>
          </div>
