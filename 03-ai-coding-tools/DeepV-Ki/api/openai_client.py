...
def get_first_message_content(completion: ChatCompletion) -> str:
    ...
    log.debug(f"Received completion (id={getattr(completion, 'id', 'unknown')})")
...
class OpenAIClient(ModelClient):
    ...
    def parse_chat_completion(
        ...
        log.debug(f"Parsing completion (id={getattr(completion, 'id', 'unknown')})")
        try:
            data = self.chat_completion_parser(completion)
        except Exception as e:
            log.error(f"Error parsing the completion: {e}")
    ...
    def track_completion_usage(
        ...
        if completion is None or not hasattr(completion, 'usage'):
            log.debug("Completion or usage is None, returning empty usage")
            return CompletionUsage(
                completion_tokens=None, prompt_tokens=None, total_tokens=None
            )

        if completion.usage is None:
            log.debug("Completion usage is None")
            return CompletionUsage(
                completion_tokens=None, prompt_tokens=None, total_tokens=None
            )

        try:
            # Try to extract cost if available (e.g. from OpenRouter)
            cost = getattr(completion.usage, 'cost', None)

            # If not found as attribute, try to find in extra fields if available
            if cost is None:
                # Check if it's a Pydantic model with extra fields
                if hasattr(completion.usage, 'model_extra') and completion.usage.model_extra:
                    cost = completion.usage.model_extra.get('cost')
                elif hasattr(completion.usage, 'extra_fields'):
                    cost = completion.usage.extra_fields.get('cost')
                # Fallback: try converting to dict if possible to see hidden fields
                elif hasattr(completion.usage, 'model_dump'):
                    try:
                        cost = completion.usage.model_dump().get('cost')
                    except:
                        pass
                elif hasattr(completion.usage, 'to_dict'):
                    try:
                        cost = completion.usage.to_dict().get('cost')
                    except:
                        pass
            ...
            # Attach cost to usage object if available
            if cost is not None:
                setattr(usage, 'cost', cost)
            else:
                # Debug log to see what's actually in usage
                try:
                    usage_debug = str(completion.usage)
                    log.debug(f"No cost found in usage: {usage_debug}")
                except:
                    pass

            return usage
        except Exception as e:
            log.debug(f"Could not extract usage info: {e}")
            return CompletionUsage(
                completion_tokens=None, prompt_tokens=None, total_tokens=None
            )
    ...
    def parse_embedding_response(
        ...
        try:
            ...
            # Log usage information if available (with graceful fallback)
            try:
                if hasattr(response, 'usage') and response.usage:
                    ...
                    # Build log message with available fields
                    log_parts = []
                    ...
                    if log_parts:
                        log.info(f"Embedding response - {', '.join(log_parts)}")
            except Exception as e:
                log.debug(f"Could not extract usage info from embedding response: {e}")
    ...
    def call(self, api_kwargs: Dict = {}, model_type: ModelType = ModelType.UNDEFINED):
        ...
        if model_type != ModelType.EMBEDDER:
            # For LLM, show the messages size before request
            msg_len = len(str(api_kwargs.get('messages', '')))
            log.info(f"Sending request to {model} (approx {msg_len} chars)")
