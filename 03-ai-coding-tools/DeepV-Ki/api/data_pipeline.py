...
def download_repo(repo_url: str, local_path: str, repo_type: str = "github", access_token: str = None, force_refresh: bool = False) -> str:
    """
    ...
    Args:
        ...
        force_refresh (bool, optional): Force refresh by deleting cached repository and re-downloading.
    ...
    try:
        ...
        logger.info(f"   - Has Token: {'YES (length: ' + str(len(access_token)) + ')' if access_token else 'NO'}")
        ...
        if os.path.exists(local_path) and os.listdir(local_path):
            ...
            if force_refresh:
                # Force refresh: delete the existing repository and re-download
                logger.warning(f"🔄 Force refresh enabled. Deleting existing repository at {local_path}")
                import shutil
                import stat

                # Windows-compatible rmtree with error handler
                ...
                logger.warning(f"⚠️ Repository already exists at {local_path}. Using existing repository.")
                return local_path
        ...
        if access_token:
            ...
            logger.info(f"🔐 Formatting clone URL with token for {repo_type}")
            ...
            if repo_type == "github":
                ...
                logger.info(f"   - GitHub format: https://TOKEN@{parsed.netloc}{parsed.path}")
            elif repo_type == "gitlab":
                # Format: https://oauth2:{token}@gitlab.com/owner/repo.git
                clone_url = urlunparse((parsed.scheme, f"oauth2:{access_token}@{parsed.netloc}", parsed.path, '', '', ''))
                logger.info(f"   - GitLab format: https://oauth2:TOKEN@{parsed.netloc}{parsed.path}")
            elif repo_type == "bitbucket":
                # Format: https://x-token-auth:{token}@bitbucket.org/owner/repo.git
                clone_url = urlunparse((parsed.scheme, f"x-token-auth:{access_token}@{parsed.netloc}", parsed.path, '', '', ''))
                logger.info(f"   - Bitbucket format: https://x-token-auth:TOKEN@{parsed.netloc}{parsed.path}")

            logger.info("✅ Using access token for authentication")
        else:
            logger.warning("⚠️ No access token provided - attempting public clone")

        # Clone the repository
        logger.info(f"🚀 Starting git clone...")
        logger.info(f"   - Command: git clone --depth=1 --single-branch [URL] {local_path}")
        ...
        logger.info(f"   - stdout: {result.stdout.decode('utf-8')[:200]}")
        return local_path
        ...
        logger.error(f"   - stdout: {stdout_msg[:500]}")

        # Sanitize error message to remove any tokens
        if access_token and access_token in error_msg:
            error_msg = error_msg.replace(access_token, "***TOKEN***")
            logger.info("   - Token sanitized from error message")

        raise ValueError(f"Error during cloning: {error_msg}")
    except Exception as e:
        logger.error(f"❌ Unexpected error during clone: {type(e).__name__}: {str(e)}")
...
def read_all_documents(path: str, is_ollama_embedder: bool = None, excluded_dirs: List[str] = None, excluded_files: List[str] = None,
    ...
    def should_process_file(file_path: str, use_inclusion: bool, included_dirs: List[str], included_files: List[str],
        ...
        Supports glob patterns like "out-*", "*.min.js", etc.
        ...
        if use_inclusion:
            ...
            # Check if file is in an included directory (with glob pattern support)
            if included_dirs:
                for included in included_dirs:
                    clean_included = included.strip("./").rstrip("/")
                    # Support glob patterns like "out-*"
                    for part in file_path_parts:
                        if fnmatch.fnmatch(part, clean_included):
                            is_included = True
                            break
                    if is_included:
                        break
            ...
            if not is_included and included_files:
                for included_file in included_files:
                    if fnmatch.fnmatch(file_name, included_file) or file_name.endswith(included_file):
                        is_included = True
                        break
            ...
            # Check if file is in an excluded directory (with glob pattern support)
            for excluded in excluded_dirs:
                clean_excluded = excluded.strip("./").rstrip("/")
                # Support glob patterns like "out-*", ".next", etc.
                for part in file_path_parts:
                    if fnmatch.fnmatch(part, clean_excluded):
                        is_excluded = True
                        break
                if is_excluded:
                    break
            ...
            if not is_excluded:
                for excluded_file in excluded_files:
                    # Support glob patterns like "*.min.js"
                    if fnmatch.fnmatch(file_name, excluded_file) or file_name == excluded_file:
                        is_excluded = True
                        break

            return not is_excluded

    # Use os.walk for efficient single-pass directory traversal
    logger.info(f"Starting efficient file scan using os.walk...")
    ...
    for root, dirs, files in os.walk(path):
        ...
        dirs[:] = [d for d in dirs if not any(excl.strip('./') == d for excl in (excluded_dirs or []))]

        for file in files:
            total_files_scanned += 1
            if total_files_scanned % 100 == 0:
                logger.info(f"📊 Scanned {total_files_scanned} files, processed {files_processed}, skipped {files_skipped}")
            ...
            try:
                with open(file_path, "r", encoding="utf-8") as f:
                    ...
                    # Determine if this is an implementation file (for code files)
                    ...
                    max_tokens = MAX_EMBEDDING_TOKENS * 10 if is_code_file else MAX_EMBEDDING_TOKENS
                    if token_count > max_tokens:
                        logger.warning(f"Skipping large file {relative_path}: Token count ({token_count}) exceeds limit ({max_tokens})")
                        files_skipped += 1
                        continue
    ...
    logger.info(f"✅ File scan complete: Total scanned={total_files_scanned}, Processed={files_processed}, Skipped={files_skipped}")
...
def transform_documents_and_save_to_db(
    ...
    logger.info(f"🔨 Preparing data transformation pipeline for {len(documents)} documents...")
    data_transformer = prepare_data_pipeline(is_ollama_embedder)

    # Save the documents to a local database
    logger.info(f"📦 Loading {len(documents)} documents into local database...")
    db = LocalDB()
    db.register_transformer(transformer=data_transformer, key="split_and_embed")
    db.load(documents)

    logger.info(f"⚙️ Generating embeddings for {len(documents)} documents (this may take a while)...")
    db.transform(key="split_and_embed")

    logger.info(f"💾 Saving transformed documents to database: {db_path}")
    os.makedirs(os.path.dirname(db_path), exist_ok=True)
    db.save_state(filepath=db_path)

    logger.info(f"✅ Database saved successfully with embeddings")
    return db
...
def get_gitlab_file_content(repo_url: str, file_path: str, access_token: str = None) -> str:
    ...
    try:
        ...
        # Handle custom ports for self-hosted GitLab instances
...
def get_file_content(repo_url: str, file_path: str, repo_type: str = "github", access_token: str = None) -> str:
    """
...
class DatabaseManager:
    ...
    def prepare_database(self, repo_url_or_path: str, type: str = "github", access_token: str = None, is_ollama_embedder: bool = None,
                       ...
                       included_dirs: List[str] = None, included_files: List[str] = None, force_refresh: bool = False) -> List[Document]:
        """
        ...
        Args:
            ...
            included_dirs (List[str], optional): List of directories to include exclusively
            included_files (List[str], optional): List of file patterns to include exclusively
            force_refresh (bool, optional): If True, delete cached database and re-generate embeddings

        Returns:
            List[Document]: List of Document objects
        """
        self.reset_database()
        self._create_repo(repo_url_or_path, type, access_token, force_refresh=force_refresh)
        return self.prepare_db_index(is_ollama_embedder=is_ollama_embedder, excluded_dirs=excluded_dirs, excluded_files=excluded_files,
                                   included_dirs=included_dirs, included_files=included_files, force_refresh=force_refresh)
    ...
    def _create_repo(self, repo_url_or_path: str, repo_type: str = "github", access_token: str = None, force_refresh: bool = False) -> None:
        """
        ...
        Args:
            repo_url_or_path (str): The URL or local path of the repository
            access_token (str, optional): Access token for private repositories
            force_refresh (bool, optional): If True, delete existing repository and re-download
        """
        ...
        try:
            ...
            if repo_url_or_path.startswith("https://") or repo_url_or_path.startswith("http://"):
                ...
                if os.path.exists(save_repo_dir) and os.listdir(save_repo_dir):
                    if force_refresh:
                        # Force refresh: delete the existing repository and re-download
                        logger.warning(f"🔄 Force refresh enabled. Deleting existing repository at {save_repo_dir}")
                        import shutil
                        import stat
                        ...
                        shutil.rmtree(save_repo_dir, onerror=remove_readonly)
                        logger.info(f"✅ Existing repository deleted successfully")
                        # Download the repository
                        download_repo(repo_url_or_path, save_repo_dir, repo_type, access_token, force_refresh=False)
                    else:
                        # Use existing repository
                        logger.info(f"⚠️ Repository already exists at {save_repo_dir}. Using existing repository.")
                else:
    ...
    def prepare_db_index(self, is_ollama_embedder: bool = None, excluded_dirs: List[str] = None, excluded_files: List[str] = None,
        ...
        # check the database
        if force_refresh:
            logger.info("🔄 Force refresh enabled: Deleting cached database and regenerating embeddings...")
            if self.repo_paths and os.path.exists(self.repo_paths["save_db_file"]):
                try:
                    os.remove(self.repo_paths["save_db_file"])
                    logger.info(f"✅ Deleted cached database: {self.repo_paths['save_db_file']}")
                except Exception as e:
                    logger.warning(f"⚠️ Failed to delete cached database: {e}")
            ...
            try:
                ...
                if documents:
                    logger.info(f"✅ Loaded {len(documents)} documents from existing database (cache hit)")
                    return documents
            except Exception as e:
        ...
        logger.info("🔨 Creating new database and generating embeddings...")
